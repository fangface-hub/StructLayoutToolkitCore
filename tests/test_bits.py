...
def test_required_bytes_for_extraction():
    """Test the _required_bytes_for_extraction function with various scenarios."""
    # offset.bit + size.bits が 8 未満 → 1 byte
...
def test_bits_get_cross_byte():
    ...
    offset = InfoSize(0, 6)  # byte0 の 6bit 目から
    size = InfoSize(0, 6)  # 6bit 抽出（byte1 に跨る）
...
def test_bits_set_single_byte():
    ...
    # 11110000 → offset=2 → xxxx → 1100 → 0011 を書く
...
def test_bits_set_cross_byte():
    ...
    # 手計算した mask と比較しても良いが、
    # bits_get で再取得して一致することを確認する方が抽象化的に美しい
    info = bits_get(buf, offset, size)
